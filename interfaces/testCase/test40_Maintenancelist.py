...
create_house_xls = commons.get_xls("caseforparame.xlsx", "Maintenancelist")
...
class Maintenancelist(unittest.TestCase):
    def setParameters(self,case_name, keywords,maxPageItems,offsetStart,status,result,response):
    ...
    def test01_Maintenancelist(self):
        # set url
        self._testMethodDoc = self.case_name
        self.url = commons.get_url_from_xml('Maintenancelist')
        if self.offsetStart == '' and self.keywords == '' and self.maxPageItems == '':
            self.url_new = self.url + "?" + "type=" + self.status
        elif self.offsetStart == '' and self.keywords != '' and self.maxPageItems != '':
            self.url_new = self.url + "?" + "&" + "keywords=" + self.keywords + "&" + "maxPageItems=" + self.maxPageItems + "&" + "type=" + self.status
        elif self.offsetStart == '' and self.keywords == '' and self.maxPageItems != '':
            self.url_new = self.url + "?" + "maxPageItems=" + self.maxPageItems + "&" + "type=" + self.status
        elif self.offsetStart != '' and self.keywords != '' and self.maxPageItems != '':
            self.url_new = self.url + "?" + "offsetStart="+self.offsetStart+"&"+"keywords="+self.keywords+"&"+"maxPageItems="+self.maxPageItems+"&"+"type="+self.status
    ...
    def checkResult(self):
        ...
        #     print('第六步，記錄響應結果')
        commons.Deposit_xls('caseforparame.xlsx', 'Maintenancelist', self.info, self.case_name)
    #         # self.assertEqual(self.info['msg'], self.msg)
    #         # self.assertEqual(email, self.email)
    #
    #     # if self.result == '1':
