...
class PostmaintenanceTasks(unittest.TestCase):
    ...
    def test01_PostmaintenanceTasks(self):

        #依賴其他接口參數
        xls_reponses = commons.Deposit_xls_relyon("caseforparame.xlsx", "PostmaintenanceTasks", "Addrepair", self.case_name)
        xls_reponse = json.loads(xls_reponses)

        # set url
        print('case ###############################################%s' % self.case_name)
        self._testMethodDoc = self.case_name
        self.url = commons.get_url_from_xml('PostmaintenanceTasks')
    ...
    def checkResult(self):
        ...
        #     print('第六步，記錄響應結果')
        commons.Deposit_xls('caseforparame.xlsx', 'PostmaintenanceTasks', self.info, self.case_name)
