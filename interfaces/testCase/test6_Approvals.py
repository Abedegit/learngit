...
create_house_xls = commons.get_xls("caseforparame.xlsx", "Postapprovals")

localReadConfig = readConfig.ReadConfig()
configHttp = ConfigHttp.ConfigHttp()
info = {}
Authorization_id = commons.get_Authorization("test88","123456","Authorizationid_approvals")

@paramunittest.parametrized(*create_house_xls)
class Postapprovals(unittest.TestCase):
    def setParameters(self,case_name, id,operation,remarks,result,response,relyon_response):
    ...
    def test01_Postapprovals(self):
        ...
        xls_reponses = commons.Deposit_xls_relyon("caseforparame.xlsx","Postapprovals","Addrepair",self.case_name)
        xls_reponse = json.loads(xls_reponses)

        # set url
        self._testMethodDoc = self.case_name
        self.url = commons.get_url_from_xml('Postapprovals')
        ...
        self.url_new = self.url + "/" + str(self.id_change) + "?" + "operation=" + self.operation + "&" + "remarks=" +self.remarks
    ...
    def checkResult(self):
        ...
        #     print('第六步，記錄響應結果')
        commons.Deposit_xls('caseforparame.xlsx', 'Postapprovals', self.info, self.case_name)
    #         # self.assertEqual(self.info['msg'], self.msg)
    #         # self.assertEqual(email, self.email)
