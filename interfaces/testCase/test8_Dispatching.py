...
create_house_xls = commons.get_xls("caseforparame.xlsx", "Postdispatching")



localReadConfig = readConfig.ReadConfig()
configHttp = ConfigHttp.ConfigHttp()
Authorization_id = commons.get_Authorization("test86","123456","Authorizationid_dispatching")
list_info = []

@paramunittest.parametrized(*create_house_xls)
class Postdispatching(unittest.TestCase):
    ...
    def test01_Postdispatching(self):

        #依賴其他接口參數
        xls_reponses = commons.Deposit_xls_relyon("caseforparame.xlsx", "Postdispatching", "Addrepair", self.case_name)
        xls_reponse = json.loads(xls_reponses)
        # set url
        print('case ###############################################%s' % self.case_name)
        self._testMethodDoc = self.case_name
        self.url = commons.get_url_from_xml('Postdispatching')
    ...
    def checkResult(self):
        ...
        #     print('第六步，記錄響應結果')
        commons.Deposit_xls('caseforparame.xlsx', 'Postdispatching', self.info, self.case_name)
